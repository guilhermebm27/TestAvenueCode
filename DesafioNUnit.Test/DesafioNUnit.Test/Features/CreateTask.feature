...
Cenário: 7) Remover tarefa e verificar se a mesma não existe na tela
	Quando acessar na tela de My Tasks
	E informar os dados para inserir nova tarefa
		| inserir nova tarefa                    |
		| teste para verificar remoção de tarefa |
	E clicar em adicionar tarefa
	Então tarefa "teste para verificar remoção de tarefa" deve ser exibida na listagem
	Quando clicar no botão Remove
	Então tarefa "teste para verificar remoção de tarefa" não deve mais existir na tela