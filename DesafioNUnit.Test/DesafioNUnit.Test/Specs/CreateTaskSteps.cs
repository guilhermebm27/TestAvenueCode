...
namespace DesafioNUnit.Specs
{
    ...
    public class CreateTaskSteps : PageBase
    {
        ...
        [When(@"clicar no botão Remove")]
        [Obsolete]
        public void QuandoClicarNoBotaoRemove()
        {
            criarTarefa.ClicarRemoverTarefa();
        }

        [Then(@"tarefa ""(.*)"" não deve mais existir na tela")]
        public void EntaoTarefaNaoDeveMaisExistirNaTela(string assert)
        {
            mc.VerificarSeTarefaFoiRemovidaCorretamente(assert);
            evidenciaTeste.Capture("BotaoRemoveFuncionando");
        }
